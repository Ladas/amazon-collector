...
RSpec.describe TopologicalInventory::Amazon::Collector::Metrics do
  ...
  context "Turned on" do
    it "exposes metrics" do
      ...
      metrics = get_metrics
      expect(metrics["topological_inventory_amazon_collector_errors_total{type=\"#{err_type}\"}"]).to eq("2")
    end
